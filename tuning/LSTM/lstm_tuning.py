...
def build_model(window_size, units, layers, dropout):
    ...
    for layer_idx in range(layers):
        ...
        if layer_idx == 0:
            model.add(LSTM(units, return_sequences = return_seq, input_shape = (window_size, 5)))
        else:
...
def plot_top(df_top, output_path):
    ...
    plt.title("LSTM Tuning - Top 5 Configurations by Average MAPE")
    ...
    plt.savefig(output_path / "lstm_tuning_top5.png", dpi = 150)
    plt.close()
...
def lstm_tuning():
    ...
    full_csv = output_path / "lstm_tuning_full.csv"
    if full_csv.exists():
    ...
    avg_ranked.to_csv(output_path / "lstm_tuning_ranked.csv", index = False)
    top5 = avg_ranked.head(5)
    top5.to_csv(output_path / "lstm_tuning_top5.csv", index = False)
