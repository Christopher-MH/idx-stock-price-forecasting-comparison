...
def plot_results(dataset, ticker_name, testing_data, real_predictions, window_size):
    context_days = 30
    prediction_dates = testing_data.index[window_size:]
...
dataset, training_data, validation_data, testing_data = load_data("ADRO.csv", processed_dataset_path)

training_scaled, validation_scaled, testing_scaled, scaler = minmax_scaling(training_data, validation_data, testing_data)

window_size = 30 # sebulan
...
X_testing, y_testing = create_windows(testing_scaled, window_size)

# Build
model = Sequential()
model.add(LSTM(64, return_sequences = True, input_shape = (window_size, 5)))
...
plot_results(dataset, "ADRO", testing_data, real_predictions, window_size)